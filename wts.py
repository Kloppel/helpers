...
class porphyr:
    ...
    def set_porphyr(self):
        ...
        for i1 in range(self.mol.size):                 # loop over all atoms
            ...
            #print(self.mol.numbers[i1])
            if self.mol.numbers[i1] == 26:              # finding FE -> n are the axial Ligands and N
            #    nofn = []
                NDict = {}
                for N in n:
                    ...
                    # it's supposed to be = {N:{C1:{C2,C2}   }} but therea still the axial ligands who have to be discovered
        ...
        for i in set(self.por_index):
            ...
            if self.graph.numbers[i]==26:
                self.calldict[i] = "Fe"
        N1  = N[0]                      # now we have all atoms of hte Heme and know if it is N, alpha-C (C1), beta-C (C2) or a methyl-C between the pyrrosl (C3). now we want to find out the orientation of the indices using the rest groups.
        C11a,C11b = 1,2                         
        ...
        for p in pairs:                                             # Here we define if the pyrrol bounds a acid or a other restgroup, thats how we want to orientate the Heme
        ...
        molmod.io.pdb.dump_pdb(filename=f"{name}.pdb", molecule=self.mol)
        # now we have a standardized heme and can figure out the angles.
        return

    def set_ordered_xyz(self):
        lines, line = [], ""
        for i in self.order:
            #print(i, self.get_symbol_per_an_single(self.mol.graph.numbers[i]), self.mol.coordinates[i])
            line += str(self.get_symbol_per_an_single(self.mol.graph.numbers[i]))
            line += str(self.mol.coordinates[i])
            line = line.replace(" ", "  ")
            line = line.replace("[", "       ")
            line = line.replace("]", "\n")
            lines.append(line)
            line = ""
        file = open(f"{self.name}_heme_sorted.xyz", "w").writelines(lines)
        return

    # Here is the part where the lists of dihedrals are maden get_dihed_per_list get a list of Index list of four indizes and calculate the dihdral angle.
